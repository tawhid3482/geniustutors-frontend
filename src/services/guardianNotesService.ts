...
class GuardianNotesService {
  ...
  async getNotes(filters: NotesFilter = {}): Promise<NotesResponse> {
    try {
      ...
      const url = `${API_BASE_URL}/guardian-notes${params.toString() ? `?${params.toString()}` : ''}`;
      ...
      console.error('Error fetching guardian notes:', error);
      throw error;
    }
  }
  ...
  async getNote(id: number): Promise<NoteResponse> {
    try {
      const response = await fetch(`${API_BASE_URL}/guardian-notes/${id}`, {
        method: 'GET',
        headers: this.getAuthHeaders()
      });
  ...
  async createNote(noteData: CreateNoteData): Promise<NoteResponse> {
    try {
      const response = await fetch(`${API_BASE_URL}/guardian-notes`, {
        method: 'POST',
        headers: this.getAuthHeaders(),
  ...
  async getCategories(): Promise<CategoriesResponse> {
    try {
      const response = await fetch(`${API_BASE_URL}/guardian-notes/categories/list`, {
        method: 'GET',
        headers: this.getAuthHeaders()
      });
  ...
  async archiveNote(id: number, isArchived: boolean): Promise<{ success: boolean; message: string }> {
    try {
      const response = await fetch(`${API_BASE_URL}/guardian-notes/${id}/archive`, {
        method: 'PATCH',
        headers: this.getAuthHeaders(),
  ...
  async toggleImportant(id: number, isImportant: boolean): Promise<{ success: boolean; message: string }> {
    try {
      const response = await fetch(`${API_BASE_URL}/guardian-notes/${id}/important`, {
        method: 'PATCH',
        headers: this.getAuthHeaders(),
...
export const guardianNotesService = new GuardianNotesService();
