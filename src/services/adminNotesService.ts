...
class AdminNotesService {
  ...
  async getNotes(filters: NotesFilter = {}): Promise<NotesResponse> {
    try {
      ...
      const url = `${API_BASE_URL}/admin-notes${params.toString() ? `?${params.toString()}` : ''}`;
  ...
  async getCategories(): Promise<CategoriesResponse> {
    try {
      const response = await fetch(`${API_BASE_URL}/admin-notes/categories/list`, {
        method: 'GET',
        headers: this.getAuthHeaders()
      });
  ...
  async archiveNote(id: number, isArchived: boolean): Promise<{ success: boolean; message: string }> {
    try {
      const response = await fetch(`${API_BASE_URL}/admin-notes/${id}/archive`, {
        method: 'PATCH',
        headers: this.getAuthHeaders(),
  ...
  async toggleImportant(id: number, isImportant: boolean): Promise<{ success: boolean; message: string }> {
    try {
      const response = await fetch(`${API_BASE_URL}/admin-notes/${id}/important`, {
        method: 'PATCH',
        headers: this.getAuthHeaders(),
